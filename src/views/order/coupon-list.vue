                ...
                <popup v-model="popup" style="background-color: #fff;">
                    <div :style="{height: popupHeight + 'px'}" style="position: relative;">
                        ...
                        <div style="margin: 10px; overflow: auto;" :style="{maxHeight: popupHeight/2 + 'px'}">
                            <span v-if="coupon.hxType == 'Category'">
                                仅限类型:
                            </span>
                            <span v-if="coupon.hxType == 'Category'" v-for="(category,index) in coupon.categories"
                                  :key="index">
                                {{category.name}}{{index < coupon.categories.length - 1 ? ', ' : ''}}
                            </span>
                            <span v-if="coupon.hxType == 'Item'">
                                仅限商品:
                            </span>
                            <span v-if="coupon.hxType == 'Item'" v-for="(item,index) in coupon.items" :key="index">
                                {{item.name}}{{index < coupon.items.length - 1 ? ', ' : ''}}
                            </span>
                            <span v-if="coupon.hxType == 'All'">
                                适用全部商品
                            </span>
                        </div>
